...
export class BubbleComponent implements OnInit, AfterContentChecked {
  ...
  public data: any[];
  public size: [number, number] = [930, 930];

  constructor() {}

  ngOnInit() {
    const diameter = 932;
    // const rootStrat = stratify();
    // const root = rootStrat(this.dataset);
    ...
    root = d3.hierarchy({ children: root }).sum((d: any) => d.value);

    const color = d3.scaleOrdinal(d3.schemePuBu);
    d3.select(this.chartWrapper.nativeElement);
    // const datum = this.chartWrapper.nativeElement.datum(this.data);
